    ...
    <div className="min-h-screen bg-slate-50/50 py-12">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="grid md:grid-cols-2 gap-10 lg:gap-16">
          <div className="h-[480px] skeleton-shimmer rounded-2xl" />
          <div className="space-y-5"><div className="h-3 skeleton-shimmer rounded w-1/4" /><div className="h-8 skeleton-shimmer rounded w-3/4" /><div className="h-4 skeleton-shimmer rounded w-full" /><div className="h-4 skeleton-shimmer rounded w-2/3" /><div className="h-12 skeleton-shimmer rounded w-1/3 mt-6" /></div>
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div className="min-h-screen bg-slate-50/50">
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
        <div className="grid md:grid-cols-2 gap-10 lg:gap-16 animate-fade-in">
          {/* Image */}
          <div className="bg-white rounded-2xl overflow-hidden border border-slate-200/80 shadow-sm">
            <div className="relative h-80 md:h-[480px] bg-slate-50">
          ...
          <div className="space-y-6">
            <div>
              <p className="text-xs font-semibold text-blue-600 uppercase tracking-wider mb-2">{product.brand}</p>
            ...
            <div className="bg-white rounded-xl p-5 border border-slate-200/80 shadow-sm space-y-5">
              <div className="flex items-end justify-between">
                ...
                <div className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-semibold ${product.stock > 0 ? 'bg-emerald-50 text-emerald-700' : 'bg-red-50 text-red-700'}`}>
                ...
                <>
                  <div>
                    ...
                    <div className="flex items-center border border-slate-200 rounded-lg bg-white w-fit">
                      <button onClick={() => setQuantity(Math.max(1, quantity - 1))} className="p-2.5 hover:bg-slate-50 transition-colors rounded-l-lg"><Minus className="w-4 h-4" /></button>
                      <Input type="number" value={quantity} onChange={(e) => setQuantity(Math.min(product.stock, Math.max(1, parseInt(e.target.value) || 1)))} className="w-14 border-0 text-center focus:ring-0 p-0 text-sm font-medium" />
                      <button onClick={() => setQuantity(Math.min(product.stock, quantity + 1))} className="p-2.5 hover:bg-slate-50 transition-colors rounded-r-lg"><Plus className="w-4 h-4" /></button>
                    </div>
                  </div>
                  <div className="flex gap-3">
                    <Button onClick={handleAddToCart} size="lg" className="flex-1 h-12 text-[15px] font-semibold bg-blue-600 hover:bg-blue-500 shadow-sm shadow-blue-600/20" disabled={adding}>
                      <ShoppingCart className="w-5 h-5 mr-2" />{adding ? 'Adding...' : 'Add to Cart'}
                    </Button>
                    <Button variant="outline" size="lg" className="h-12 w-12 p-0 border-slate-200" onClick={() => setWishlisted(!wishlisted)}>
            ...
            <div className="grid grid-cols-3 gap-3">
              {[
                ...
                <div key={b.label} className="flex flex-col items-center text-center p-3 bg-white rounded-xl border border-slate-200/80">
                  <b.icon className="w-5 h-5 text-blue-600 mb-1.5" />
                ...
                <CardContent>
                  <dl className="space-y-0">
                    {Object.entries(product.specs).map(([key, value], index) => (
                      <div key={key} className={`flex justify-between py-2.5 text-sm ${index !== Object.entries(product.specs).length - 1 ? 'border-b border-slate-100' : ''}`}>
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
              {related.map((p) => (
                <Link key={p.id} href={`/products/${p.id}`}>
                  <div className="group rounded-xl overflow-hidden border border-slate-200/80 bg-white hover:shadow-lg transition-all duration-300 hover:-translate-y-0.5">
                    ...
                    <div className="p-3 space-y-1">
                      <p className="text-[10px] font-semibold text-slate-400 uppercase">{p.brand}</p>
                      <h3 className="font-medium text-slate-900 text-sm line-clamp-1 group-hover:text-blue-600 transition-colors">{p.name}</h3>
                      <p className="text-sm font-bold text-slate-900">${p.price.toFixed(2)}</p>
                    </div>
                  </div>
                </Link>
              ))}
